...
void get(const std::string& initial,uint64_t limit,std::stop_token stop,
    ...
    Internet session(WinHttpOpen(wide(std::string("PSVR2SimShaker/")+appVersion).c_str(),WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,
...
bool Release::installable()const{
    ...
    return installerName=="PSVR2SimShaker-"+fileVersion+"-Setup.exe" &&
...
std::optional<Release> newestRelease(const Json& list){
    ...
    for(const auto& item:list){
        try{
            ...
            const auto expected="PSVR2SimShaker-"+r.version+"-Setup.exe";
...
UpdateClient::UpdateClient(){check();}
